...
def apply_sfbc(symbols):
    """
    Aplica Space-Frequency Block Coding (SFBC) para diversidad en TX con 2 antenas.
    
    Args:
        symbols (numpy array): Símbolos OFDM a transmitir.
    
    Returns:
        tuple: Símbolos codificados para las 2 antenas.
    """
    # SFBC para 2 antenas
    symbols_ant1 = symbols  # Antena 1 transmite los símbolos originales
    symbols_ant2 = np.roll(np.conj(symbols), -1)  # Antena 2 transmite los símbolos conjugados y desplazados
    
    return symbols_ant1, symbols_ant2

def apply_beamforming(symbols, num_antennas, angle):
    """
    Aplica beamforming en TX con 4 a 8 antenas.
    
    Args:
        symbols (numpy array): Símbolos OFDM a transmitir.
        num_antennas (int): Número de antenas (4 u 8).
        angle (float): Ángulo de direccionamiento del haz en grados.
    
    Returns:
        numpy array: Símbolos con beamforming aplicado.
    """
    # Convertir el ángulo a radianes
    theta = np.deg2rad(angle)
    
    # Crear el vector de beamforming
    d = 0.5  # Espaciado entre antenas en longitudes de onda
    n = np.arange(num_antennas)
    steering_vector = np.exp(-1j * 2 * np.pi * d * n * np.sin(theta))
    
    # Aplicar beamforming a los símbolos
    symbols_beamformed = np.outer(symbols, steering_vector)
    
    return symbols_beamformed
...
def main():
    ...
    symbols_ant1, symbols_ant2 = apply_sfbc(symbols)
    
    # Aplicar DFT a los símbolos modulados (SC-FDM) para cada antena
    symbols_freq_ant1 = apply_dft(symbols_ant1)
    symbols_freq_ant2 = apply_dft(symbols_ant2)
    
    # Dividir los símbolos modulados en bloques según el número de subportadoras de datos
    grupos_resultantes_ant1 = Serie_paralelo(symbols_freq_ant1, abs(nc - len(pilotCarriers)))
    grupos_resultantes_ant2 = Serie_paralelo(symbols_freq_ant2, abs(nc - len(pilotCarriers)))
    
    # Procesar símbolos para la antena 1
    ofdm_canal_ant1 = []
    for grupo in grupos_resultantes_ant1:
        OFDM_data = OFDM_symbol(grupo, pilotCarriers, dataCarriers, pilotValue, n)
        OFDM_time = IDFT(OFDM_data)
        OFDM_withCP = addCP(OFDM_time, CP)
    ...
    # Procesar símbolos para la antena 2
    ofdm_canal_ant2 = []
    for grupo in grupos_resultantes_ant2:
        OFDM_data = OFDM_symbol(grupo, pilotCarriers, dataCarriers, pilotValue, n)
        OFDM_time = IDFT(OFDM_data)
        OFDM_withCP = addCP(OFDM_time, CP)
        # Definir noise_prob antes de llamar a channel
    ...
    combined_signals = []
    for rx_ant1, rx_ant2 in zip(ofdm_canal_ant1, ofdm_canal_ant2):
