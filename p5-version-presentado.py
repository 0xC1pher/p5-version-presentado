...
def image_to_bits_bw(image):
    ...
    # print(array[:2])    # imprime las dos primeras filas de la matriz imagen
    # print(len(array))   # numero de filas de matriz de imagen
    ...
    # print(bits[8:16])   # imprime desde el bit 8 al bit 16, es decir el valor del segundo pixel en binario de 8 bits
...
def main():
    ...
    # # Mostrar información sobre los bits
    print(bitsbn[:20])  # Mostrar solo los primeros 100 bits para ver el resultado
    
    # Solicitar parámetros del ancho de banda y el espaciado
    bw, delta_f = get_bandwidth_and_spacing()
    
    # Solicita el valor de SNR en dB
    SNRdb = get_snr()  # Relación señal-ruido en dB

    # Obtener el prefijo ciclico
    opcion = get_longitud_CP()
    if opcion == 1:
        T_CP=4.6e-6
        CP=round(T_CP*(delta_f))
    elif opcion == 2 and delta_f == 15000.0:
        T_CP=16.6e-6
        CP=round(T_CP*(delta_f))
    elif opcion == 2 and delta_f == 7500.0:
        T_CP=33e-6   
        CP=round(T_CP*(delta_f))
        
    # Calcular el número de subportadoras activas
    nc_calculado = calculate_nc(bw, delta_f)
    nc=math.ceil(nc_calculado)
    
    print(f"Número de subportadoras activas (N_c): {nc}")
    
    # Calcular el tamaño de la IFFT (N) como potencia de 2: N>Nc
    n = calculate_ifft_size(nc)
    print(f"Tamaño de la IFFT (N): {n}")
    
    # Pedir al usuario el porcentaje de subportadoras piloto
    while True:
        try:
            porcentaje_pilotos = float(input("Ingrese el porcentaje de subportadoras piloto (ejemplo: 5.0 para 5%): "))
            if 0 < porcentaje_pilotos < 100:
                break
            else:
    ...
    # Configuración de subportadoras
    pilotValue = -1-1j # The known value each pilot transmits

    # Índices de todas las subportadoras
    allCarriers = np.arange(nc)  # Total de subportadoras (N)
        
    print(f"Subportadoras piloto: {pilotCarriers}")
    print(f"Subportadoras de datos: {dataCarriers}")
    
    print(f"longitud portadoras total: {len(allCarriers)}")
    print(f"longitud portadoras piloto: {len(pilotCarriers)}")
    print(f"longitud portadoras datos: {len(dataCarriers)}")
    
    # Configuración del canal
    channelResponse = np.array([1, 0, 0.3 + 0.3j])  # Respuesta impulsiva
    H_exact = np.fft.fft(channelResponse, nc)       # Espectro del canal
    ...
    symbols = modulate_with_mapping(bitsbn, modulation_type)
...
Asignación de portadoras pilotos
...
Cálculo del BER
Gráfica de BER vs SNR para las graficas 
