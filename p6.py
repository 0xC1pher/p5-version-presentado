...
def modular_scfdm(datos, portadoras_piloto, portadoras_datos, valor_piloto, n):
    """
    Modula los datos en un símbolo SC-FDM (aplica DFT antes de OFDM).
    ...
    Returns:
        numpy array: Símbolo SC-FDM.
    """
    # Aplicar DFT a los datos
    datos_dft = np.fft.fft(datos)
    # Modular OFDM con los datos en el dominio de la frecuencia
    simbolo_scfdm = modular_ofdm(datos_dft, portadoras_piloto, portadoras_datos, valor_piloto, n)
    return simbolo_scfdm
...
def demodular_scfdm(senal_recibida, portadoras_piloto, valor_piloto, portadoras_datos, n):
    """
    Demodula un símbolo SC-FDM (aplica IDFT después de OFDM).
    ...
    """
    # Demodular OFDM
    simbolo_ofdm = demodular_ofdm(senal_recibida, portadoras_piloto, valor_piloto, portadoras_datos, n)
    # Aplicar IDFT para volver al dominio del tiempo
    simbolo_scfdm = np.fft.ifft(simbolo_ofdm)
    return simbolo_scfdm
...
def calcular_papr(senal):
    """
    Calcula la Relación de Potencia de Pico a Promedio (PAPR) de una señal.

    Args:
        senal (numpy array): Señal en el dominio del tiempo.

    Returns:
        float: PAPR en dB.
    """
    potencia_pico = np.max(np.abs(senal)**2)
    potencia_promedio = np.mean(np.abs(senal)**2)
    papr = 10 * np.log10(potencia_pico / potencia_promedio)
    return papr

def calcular_ccdf_papr(senal, num_muestras=1000):
    """
    Calcula la CCDF (Complementary Cumulative Distribution Function) del PAPR.

    Args:
        senal (numpy array): Señal en el dominio del tiempo.
        num_muestras (int): Número de muestras para calcular la CCDF.

    Returns:
        tuple: (papr_values, ccdf), donde papr_values son los valores de PAPR
               y ccdf es la probabilidad de que el PAPR exceda un valor dado.
    """
    papr_values = np.zeros(num_muestras)
    for i in range(num_muestras):
        senal_ruidosa = canal(senal, snr_db=30)  # Simular ruido
        papr_values[i] = calcular_papr(senal_ruidosa)
    papr_sorted = np.sort(papr_values)
    ccdf = 1 - np.arange(num_muestras) / num_muestras
...
def main():
    ...
    senal_transmitida_ofdm = np.concatenate((cp, senal_ofdm))

    # Modulación SC-FDM
    senal_scfdm = modular_scfdm(simbolos_modulados, portadoras_piloto, portadoras_datos, valor_piloto, n)
    cp_scfdm = senal_scfdm[-longitud_cp:]
    senal_transmitida_scfdm = np.concatenate((cp_scfdm, senal_scfdm))

    # Simulación del canal para OFDM
    senal_recibida_ofdm = canal(senal_transmitida_ofdm, snr_db)
    senal_sin_cp_ofdm = senal_recibida_ofdm[longitud_cp:]  # Eliminar el prefijo cíclico
    simbolos_demodulados_ofdm = demodular_ofdm(senal_sin_cp_ofdm, portadoras_piloto, valor_piloto, portadoras_datos, n)
    bits_decodificados_ofdm = demapear_simbolos(simbolos_demodulados_ofdm, tipo_modulacion)
    if isinstance(bits_decodificados_ofdm, str):
        print(bits_decodificados_ofdm)
        return

    # Simulación del canal para SC-FDM
    senal_recibida_scfdm = canal(senal_transmitida_scfdm, snr_db)
    senal_sin_cp_scfdm = senal_recibida_scfdm[longitud_cp:]  # Eliminar el prefijo cíclico
    simbolos_demodulados_scfdm = demodular_scfdm(senal_sin_cp_scfdm, portadoras_piloto, valor_piloto, portadoras_datos, n)
    bits_decodificados_scfdm = demapear_simbolos(simbolos_demodulados_scfdm, tipo_modulacion)
    if isinstance(bits_decodificados_scfdm, str):
        print(bits_decodificados_scfdm)
        return

    # Reconstrucción de la imagen para OFDM
    imagen_reconstruida_ofdm = reconstruir_imagen_desde_bits(bits_decodificados_ofdm, ancho, alto)

    # Reconstrucción de la imagen para SC-FDM
    imagen_reconstruida_scfdm = reconstruir_imagen_desde_bits(bits_decodificados_scfdm, ancho, alto)
    ...
    plt.subplot(1, 3, 3)
    plt.imshow(imagen_reconstruida_ofdm, cmap='gray')
    plt.title('Imagen Reconstruida (OFDM)')
    plt.axis('off')

    plt.tight_layout()
    plt.show()

    # Gráfica de CCDF de PAPR
    papr_ofdm, ccdf_ofdm = calcular_ccdf_papr(senal_transmitida_ofdm)
    papr_scfdm, ccdf_scfdm = calcular_ccdf_papr(senal_transmitida_scfdm)

    plt.figure()
    plt.semilogy(papr_ofdm, ccdf_ofdm, label='OFDM')
    plt.semilogy(papr_scfdm, ccdf_scfdm, label='SC-FDM')
    plt.xlabel('PAPR (dB)')
    plt.ylabel('CCDF')
    plt.title('CCDF de PAPR para OFDM y SC-FDM')
    plt.grid(True)
    plt.legend()
    plt.show()
